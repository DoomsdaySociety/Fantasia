...
public class GuiServerEntry implements GuiListExtended.IGuiListEntry {
    ...
    private final GuiServerList owner;
    ...
    public void drawEntry(int slotIndex, int x, int y, int listWidth, int slotHeight, int mouseX, int mouseY, boolean isSelected, float partialTicks) {
        ...
        this.mc.fontRenderer.drawString(this.server.serverName, x + 32 + 3, y + 3, 16777215);

        String s2 = this.server.pingToServer != -2L ? (flag2 ? TextFormatting.DARK_RED + this.server.gameVersion : this.server.populationInfo) : server.serverMOTD;
        this.mc.fontRenderer.drawString(s2, x + 32 + 3, y + 15, 8421504);
        int k = 0;
        int l = 5;
        String s1;
        ...
        this.drawTextureAt(x, y, this.icon != null ? this.serverIcon : UNKNOWN_SERVER);
    ...
    public boolean mousePressed(int slotIndex, int mouseX, int mouseY, int mouseEvent, int relativeX, int relativeY) {
        ...
        if (this.owner.isShowSelectionBox())
            this.owner.setSelectedSlotIndex(slotIndex);
        else {
            this.owner.setShowSelectionBox(true);
            this.owner.setDimensions(this.owner.width, this.owner.height, this.owner.x, this.owner.y);
        }
